...
async def list_email_templates(current_user: dict = Depends(get_current_user_or_system_owner)):
    return await EmailTemplateService.get_all_templates()


@router.post("", response_model=dict)
async def create_email_template(template: EmailTemplateCreate, current_user: dict = Depends(get_current_user_or_system_owner)):
    template_dict = template.model_dump()
    return await EmailTemplateService.create_template(template_dict)


@router.get("/{template_id}", response_model=dict)
async def get_email_template(template_id: str, current_user: dict = Depends(get_current_user_or_system_owner)):
...
async def update_email_template(template_id: str, template: EmailTemplateCreate, current_user: dict = Depends(get_current_user_or_system_owner)):
...
async def delete_email_template(template_id: str, current_user: dict = Depends(get_current_user_or_system_owner)):
